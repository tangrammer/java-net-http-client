;; Starting point code taken from https://github.com/technomancy/clojure-http-client

(ns java-net-http-client.client
...
representation of argument, either a string or map."
  [arg]
  (if (map? arg)
    (clojure.string/join \& (map #(str/join \= (map url-encode %)) arg))
    (URLEncoder/encode (str arg) #_(as-str arg) "UTF-8")))
...
or the error stream of connection, whichever is appropriate."
  ...
  (with-open [r (clojure.java.io/reader (or (if (>= (.getResponseCode connection) 400)
                       (.getErrorStream connection)
                       (.getInputStream connection))
                     (StringReader. "")))]
    (doall (map identity  (line-seq r)))
    ))
...
\"Cookie\" header when given a clojure map of cookies."
  ...
  (str/join "; " (map (fn [cookie]
                        (str #^String (str (key cookie))
                             "="
                             #^String (str (val cookie))))
                      cookie-map)))
...
(defn request
  ...
  (let [u (url u)
        ...
        method (.toUpperCase #^String (str (or method
                                                  "GET")))]
